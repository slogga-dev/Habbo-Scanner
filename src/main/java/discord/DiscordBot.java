...
public class DiscordBot extends ListenerAdapter {
    ...
    public void onMessageReceived(MessageReceivedEvent event) {
        boolean discordBotAllowRandomPhrases = Boolean.parseBoolean(HabboScanner.getInstance()
                .getDiscordProperties().getProperty("discord.bot.allow.random.phrases"));
    ...
    public int getHabboIdFromDiscordId(String discordId){
        String discordToHabboIdMapping = HabboScanner.getInstance()
                .getDiscordProperties().getProperty("discord.to.habbo.id.mapping");
    ...
    public void sendMessageToFeedChannel(String message) {
        String feedChannelId = HabboScanner.getInstance().getDiscordProperties()
    ...
    private void initializeDiscordAPI() {
        String token = HabboScanner.getInstance().getDiscordProperties()
                .getProperty("discord.bot.token");

        String readyMessage = HabboScanner.getInstance().getMessageProperties()
    ...
    private void setupStartCommand() {
        String description = HabboScanner.getInstance().getCommandDescriptionProperties()
    ...
    private void setupFollowCommand() {
        Properties commandDescriptionProperties = HabboScanner.getInstance().getCommandDescriptionProperties();
    ...
    private void sendRandomPhrase(MessageReceivedEvent event) {
        ...
        int responseInterval = Integer.parseInt(HabboScanner.getInstance()
                .getDiscordProperties().getProperty("discord.bot.random.phrases.response.interval"));

        int randomMessageSendThreshold = random.nextInt(responseInterval) + 1;

        String phrasesList = HabboScanner.getInstance()
                .getDiscordProperties().getProperty("discord.bot.random.phrases.list");
